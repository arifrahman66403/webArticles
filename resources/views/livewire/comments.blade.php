        ...
        <div class="flex gap-3 py-3 border-b border-gray-100 text-sm">
            ...
            <div class="flex-1">
                ...
                <div class="mt-1 flex items-center gap-4 text-xs text-gray-500">
                    <button @click="openReply = openReply === {{ $comment->id }} ? null : {{ $comment->id }}"
                    ...
                    @if($comment->user_id === auth()->id())
                        <button wire:click="deleteComment({{ $comment->id }})"
                            ...
                            class="hover:underline text-red-500">Delete</button>
                    @endif
                </div>

                <!-- Form reply -->
                <div x-show="openReply === {{ $comment->id }}" x-transition class="mt-2">
                    <form wire:submit.prevent="postComment({{ $comment->id }}, {{ $comment->user_id }})">
                        <textarea wire:model.defer="replyBodies.{{ $comment->id }}"
                            class="w-full border rounded-lg px-3 py-2 text-sm focus:ring focus:ring-blue-200"
                        ...
                        @foreach($comment->replies as $reply)
                            <div class="flex gap-2">
                                ...
                                <div class="flex-1">
                                    ...
                                    <div class="mt-1 flex items-center gap-4 text-xs text-gray-500">
                                        <button @click="openReply = openReply === {{ $reply->id }} ? null : {{ $reply->id }}"
                                                class="hover:underline">Reply</button>
                                    ...
                                    <!-- Nested reply form -->
                                    <div x-show="openReply === {{ $reply->id }}" x-transition class="mt-2">
                                        <form wire:submit.prevent="postComment({{ $reply->id }}, {{ $reply->user_id }})">
                                            <textarea wire:model.defer="replyBodies.{{ $reply->id }}"
                                                class="w-full border rounded-lg px-3 py-2 text-sm focus:ring focus:ring-blue-200"
                                                placeholder="Write a reply..."></textarea>
